...
from sinobot_ctpn.utils.bbox import bbox

from sinobot_ctpn.utils.bbox import bbox_transform
from sinobot_ctpn.utils.rpn_msr import config
from sinobot_ctpn.utils.rpn_msr import generate_anchors
...
def anchor_target_layer(rpn_cls_score, gt_boxes, im_info, _feat_stride=[16, ], anchor_scales=[16, ]):
    ...
    """
    _anchors = generate_anchors.generate_anchors(scales=np.array(anchor_scales))  # 生成基本的anchor,一共9个
    ...
    if not config.Config.RPN_CLOBBER_POSITIVES:
        # assign bg labels first so that positive labels can clobber them
        labels[max_overlaps < config.Config.RPN_NEGATIVE_OVERLAP] = 0  # 先给背景上标签，小于0.3overlap的

    # fg label: for each gt, anchor with highest overlap
    labels[gt_argmax_overlaps] = 1  # 每个位置上的9个anchor中overlap最大的认为是前景
    # fg label: above threshold IOU
    labels[max_overlaps >= config.Config.RPN_POSITIVE_OVERLAP] = 1  # overlap大于0.7的认为是前景

    if config.Config.RPN_CLOBBER_POSITIVES:
        # assign bg labels last so that negative labels can clobber positives
        labels[max_overlaps < config.Config.RPN_NEGATIVE_OVERLAP] = 0

    # subsample positive labels if we have too many
    # 对正样本进行采样，如果正样本的数量太多的话
    # 限制正样本的数量不超过128个
    num_fg = int(config.Config.RPN_FG_FRACTION * config.Config.RPN_BATCHSIZE)
    ...
    num_bg = config.Config.RPN_BATCHSIZE - np.sum(labels == 1)
    ...
    bbox_inside_weights[labels == 1, :] = np.array(config.Config.RPN_BBOX_INSIDE_WEIGHTS)  # 内部权重，前景就给1，其他是0

    bbox_outside_weights = np.zeros((len(inds_inside), 4), dtype=np.float32)
    if config.Config.RPN_POSITIVE_WEIGHT < 0:  # 暂时使用uniform 权重，也就是正样本是1，负样本是0
        ...
        assert ((config.Config.RPN_POSITIVE_WEIGHT > 0) &
                (config.Config.RPN_POSITIVE_WEIGHT < 1))
        positive_weights = (config.Config.RPN_POSITIVE_WEIGHT /
                            (np.sum(labels == 1)) + 1)
        negative_weights = ((1.0 - config.Config.RPN_POSITIVE_WEIGHT) /
                            (np.sum(labels == 0)) + 1)
