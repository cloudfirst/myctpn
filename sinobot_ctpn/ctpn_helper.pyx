...
import numpy as np
from sinobotocr import my_log
from sinobot_ctpn.utils.rpn_msr import proposal_layer
from sinobot_ctpn.utils.text_connector import detectors
...
def get_ctpn_boxes(table_img):
    ...
    textsegs, _ = proposal_layer.proposal_layer(cls_prob_val, bbox_pred_val, im_info)
    scores = textsegs[:, 0]
    textsegs = textsegs[:, 1:5]

    textdetector = detectors.TextDetector(DETECT_MODE='H')
