# -*- coding:utf-8 -*-
import numpy as np
from sinobot_ctpn.utils.bbox import nms
...
def proposal_layer(rpn_cls_prob_reshape, rpn_bbox_pred, im_info, _feat_stride=[16, ], anchor_scales=[16, ]):
    ...
    _anchors = generate_anchors.generate_anchors(scales=np.array(anchor_scales))  # 生成基本的9个anchor
    ...
    pre_nms_topN = config.Config.RPN_PRE_NMS_TOP_N  # 12000,在做nms之前，最多保留的候选box数目
    post_nms_topN = config.Config.RPN_POST_NMS_TOP_N  # 2000，做完nms之后，最多保留的box的数目
    nms_thresh = config.Config.RPN_NMS_THRESH  # nms用参数，阈值是0.7
    min_size = config.Config.RPN_MIN_SIZE  # 候选box的最小尺寸，目前是16，高宽均要大于16
    ...
    proposals = bbox_transform.bbox_transform_inv(anchors, bbox_deltas)  # 做逆变换，得到box在图像上的真实坐标

    # 2. clip predicted boxes to image
    proposals = bbox_transform.clip_boxes(proposals, im_info[:2])  # 将所有的proposal修建一下，超出图像范围的将会被修剪掉
    ...
    keep = nms.nms(np.hstack((proposals, scores)), nms_thresh)  # 进行nms操作，保留2000个proposal
    if post_nms_topN > 0:
